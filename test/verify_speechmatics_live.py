...
API_KEY = os.getenv("SPEECHMATICS_API_KEY", "Lpv6UaeaM5r0k80NKNm7vhSf89340cmF")
...
async def run_live_acoustic_benchmark():
    ...
    print(f"[*] API Key Present:  {API_KEY[:4]}...{API_KEY[-4:]} (Active)")
    ...
    try:
        async with websockets.connect(WS_URL, additional_headers=headers, open_timeout=20.0) as ws:
            ...
            # 3. Stream 1 second of 16kHz 16-bit PCM audio frames
            sample_rate = 16000
            duration_s = 0.5
            total_samples = int(sample_rate * duration_s)
            
            # Generate synthetic PCM s16le silence/audio
